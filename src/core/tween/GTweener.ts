...
import { evaluateEase } from './EaseManager.js';
import type { GPath } from './GPath.js';
import { TweenValue } from './TweenValue.js';

/**
 * Duck-typed stand-in for the reference's `cc.Node` type test: anything whose
 * liveness `isNodeAlive` can decide is treated as an engine node.
 */
function isNodeLike(value: any): boolean {
    if (value == null)
        return false;
    return typeof value.isDisposed === 'function'
        || typeof value.isValid === 'boolean'
        || typeof value.disposed === 'boolean';
}

/**
 * Duck-typed stand-in for the reference's `cc.isValid(node)`. The runtime never
 * imports an engine node type, so liveness is read off whichever flag the host
 * object exposes (Babylon's `isDisposed()`, Cocos' `isValid`, our own
 * `disposed`); objects with none are assumed alive.
 */
function isNodeAlive(node: any): boolean {
    if (node == null)
        return false;
    if (typeof node.isDisposed === 'function')
        return !node.isDisposed();
    if (typeof node.isValid === 'boolean')
        return node.isValid;
...
export class GTweener {
    ...
    /** Liveness handle for the scene node a target wraps, when it has one. */
    public _node: any = null;
    ...
    private _easeType: number = EaseType.QuadOut;
    ...
    public setTarget(value: any, propType?: any): GTweener {
        ...
        if (value != null) {
            // A wrapper such as GObject hands over the node it draws into.
    ...
    public _update(dt: number): void {
        if (this._node && !isNodeAlive(this._node)) {
            this._killed = true;
            return;
        }
    ...
    private update(): void {
        ...
        if (this._repeat != 0) {
            const round: number = Math.floor(tt / this._duration);
        ...
        if (this._valueSize == 6) {
            if (this._ended == 0) {
                const r: number = this._startValue.w * (1 - this._normalizedTime);
                const rx: number = r * (Math.random() > 0.5 ? 1 : -1);
                const ry: number = r * (Math.random() > 0.5 ? 1 : -1);
            ...
            for (let i: number = 0; i < cnt; i++) {
                const n1: number = this._startValue.getField(i);
                const n2: number = this._endValue.getField(i);
